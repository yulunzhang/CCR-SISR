function [lores hires] = collectSamplesScales_trueLR(conf, ohires, numscales, scalefactor)
...
for scale = 1:numscales
    ...
    features_bic = collect(conf, midres, conf.upsample_factor, conf.filters);
    features_trueLR = collect(conf, chires, conf.upsample_factor, conf.filters);
    clear midres
    ...
    lores = [lores conf.V_pca' * features_bic conf.V_pca' * features_trueLR];
end