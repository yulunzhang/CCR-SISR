...
input_dir = 'Setchan1'; % Directory with input images from Set5 image dataset
...
for d = 10   %1024
    ...
    mat_file = ['tconf_Zeyde_' num2str(dict_sizes(d)) '_finalx' num2str(upscaling)];    
...
%         'NE+LS','NE+NNLS','NE+LLE','Our A+ (0.5mil)','Our A+', 'Our A+ (16atoms)'};
    conf.desc = {'Gnd', 'Bicubic', 'Yang', 'Zeyde', 'GR', 'ANR', 'NE_LS','NE_NNLS','NE_LLE','Aplus05mil','Aplus', 'Aplus16atoms', 'App_Zhang', 'KMSE', 'KMSE_sub'};
    conf.results = {};
...
%         %%%%%%%%%%%  SR via KMSE_abs  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         if ~isempty(KMSE_abs_PPs)
%             fprintf('KMSE_abs\n');
%             conf.PPs = KMSE_abs_PPs;
%             startt = tic;
%             
%             conf.pointslo = loaddata_abs.centers(:,conf.points);
%             res{14} = scaleup_APP_Zhang_MIKSVD(conf, low);
%             toc(startt)
%             conf.countedtime(14,i) = toc(startt);
%             conf.pointslo = conf.dict_lores(:,conf.points);
%         else
...
%         %%%%%%%%%%%  SR via KMSE_Euc  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         if ~isempty(KMSE_Euc_PPs)
%             fprintf('KMSE_Euc\n');
%             conf.PPs = KMSE_Euc_PPs;
%             startt = tic;
%             
%             conf.pointslo = loaddata_Euc.centers(:,conf.points);
...
%         end
        ...
        result = cat(3, img{1}, interpolated{1}, res{2}{1}, res{3}{1}, ...
            ...
            res{9}{1}, res{10}{1}, res{11}{1}, res{12}{1}, res{13}{1}, res{14}{1});
...
%             conf.resultsRGB{i}{j} = fullfile(conf.result_dirImagesRGB, [n sprintf('[%d-%s]', j, conf.desc{j}) x]);
    ...
    % Test performance
%     scores = run_comparison_PSNR(conf);
%     run_comparison_SSIM(conf);
